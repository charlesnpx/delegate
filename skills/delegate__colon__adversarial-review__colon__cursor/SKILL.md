...
Use this when an orchestrator should delegate a read-only refute-first adversarial code review to the cursor backend through delegate's sanitized review-context pipeline and return immediately with the launch envelope.
...
~~~bash
delegate adversarial-review --backend cursor --origin delegate:adversarial-review:cursor --cwd "$PWD" --background --json
~~~
