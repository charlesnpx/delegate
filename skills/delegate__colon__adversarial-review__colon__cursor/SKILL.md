...
description: Delegate a refute-first adversarial code review to cursor through sanitized delegate review context and return the submit receipt verbatim.
version: v0.9.1
---

# delegate:adversarial-review:cursor

Use this when an orchestrator should delegate a read-only refute-first adversarial code review to the cursor backend through delegate's sanitized review-context pipeline and return immediately with the submit receipt.
...
~~~bash
delegate adversarial-review --backend cursor --cwd "$PWD"
~~~
