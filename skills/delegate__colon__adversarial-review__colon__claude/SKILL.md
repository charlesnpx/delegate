...
Use this when an orchestrator should delegate a read-only refute-first adversarial code review to the claude backend through delegate's sanitized review-context pipeline and return immediately with the launch envelope.
...
- shared fs: the parent harness, "delegate", agentbus, and the claude backend can see the delegate state path. Using the private workspace as backend cwd is not OS isolation; a same-user backend can still read repository or other filesystem files when process permissions allow it.
...
~~~bash
delegate adversarial-review --backend claude --origin delegate:adversarial-review:claude --cwd "$PWD" --background --json
~~~
