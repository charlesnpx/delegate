...
- exec: "delegate", "agentbus", "git", and the claude backend executable are runnable. Git is used by host-side delegate assembly only; it is not a review-worker preflight or input source.
...
- backend reachability: "delegate setup --json --backend claude" shows agentbus capabilities and claude backend availability without unrelated backend model catalogues.
