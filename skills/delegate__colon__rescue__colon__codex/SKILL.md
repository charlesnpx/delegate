...
description: Delegate a rescue task to codex through delegate and return the submit receipt verbatim.
version: v0.9.1
---

# delegate:rescue:codex

Use this when an orchestrator should delegate a rescue task to the codex backend through "delegate" and return immediately with the submit receipt.
...
~~~bash
printf '%s' "$PROMPT" | delegate task --backend codex --cwd "$PWD" --prompt-file - --tag "skill=delegate:rescue:codex"
~~~
