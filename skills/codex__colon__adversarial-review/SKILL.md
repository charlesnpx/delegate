...
Use this when Claude Code should delegate a read-only refute-first adversarial code review to Codex through delegate's sanitized review-context pipeline and return immediately with the launch envelope.
...
- shared fs: Claude Code, "delegate", agentbus, and Codex can see the delegate state path; the backend does not need the live repository in safe mode.
...
~~~bash
delegate adversarial-review --backend codex --origin codex:adversarial-review --cwd "$PWD" --background --json
~~~
