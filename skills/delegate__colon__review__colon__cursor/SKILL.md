...
Use this when an orchestrator should delegate a read-only code review to the cursor backend through delegate's sanitized review-context pipeline and return immediately with the launch envelope.
...
~~~bash
delegate review --backend cursor --origin delegate:review:cursor --cwd "$PWD" --background --json
~~~
