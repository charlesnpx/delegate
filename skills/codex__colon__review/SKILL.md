...
Use this when Claude Code should delegate a read-only code review to Codex through delegate's sanitized review-context pipeline and return immediately with the launch envelope.
...
~~~bash
delegate review --backend codex --origin codex:review --cwd "$PWD" --background --json
~~~
