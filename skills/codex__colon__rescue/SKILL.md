---
name: codex:rescue
description: Delegate a rescue task from Claude Code to Codex through delegate and return the launch envelope verbatim.
version: v0.3.0
---

# codex:rescue

Use this when Claude Code should delegate a rescue task to Codex through "delegate" and return immediately with the launch envelope.
...
- shared fs: Claude Code, "delegate", agentbus, and Codex can see the same repo path and delegate state.
...
~~~bash
delegate task --backend codex --origin codex:rescue --cwd "$PWD" --handoff-prompt-file "$HANDOFF_PATH" --background --json
~~~
