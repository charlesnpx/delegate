...
Without `--job`, `status` lists summaries from every workspace unless filtered. Its optional filters are `--workspace-key`, repeatable `--tag`, and repeatable `--state`, and different filters combine with AND, while `--job` selects one record. Repeating a flag differs by flag: repeated `--tag` requires every tag to match, whereas repeated `--state` matches any listed state. Use the submit receipt's `workspaceKey` value verbatim for `--workspace-key`. Valid states are `queued`, `running`, `completed`, `failed`, `canceled`, and `unknown`. A summary has no result; list to select, then inspect with `--job`. Without a kind filter, `transcript` returns a digest; use `--kind message` for only the agent's messages and `--last <n>` to bound recent items.
