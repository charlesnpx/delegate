...
Review workspaces are private Delegate-state directories. Delegate records only the job ID, workspace path, and Agentbus state root. On the next `review` or `adversarial-review` invocation, it makes one Agentbus status read for each retained workspace and removes the workspace and its record after a terminal state. It does not wait or poll; a running or unreadable job is checked again on a later review invocation.
